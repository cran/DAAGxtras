`datafile` <-
function(file=c("fuel", "travelbooks"), datastore=DAAGxdb, altstore=zzDAAGxdb,
           showNames=FALSE){
    if(!is.null(file)) if(file[1]=="")file <- NULL
    ...
    if(is.null(file))return(c(storenames, altnames))
    for(nam in file){
      fnam <- nam
      ...
      if(extension=="csv")elname <- paste(stem, ".csv", sep="") else elname <- stem
      if(stem %in% altnames){
        if(stem=="bostonc"){
          cat(altstore[[stem]][1:9], file = "bostonc.txt",
              sep = "\t", fill = TRUE)
          cat("\n", file = "bostonc.txt", sep = "\t",
              append = TRUE)
